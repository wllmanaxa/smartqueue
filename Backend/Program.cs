...
{
    var corsSettings = app.Configuration.GetSection(CorsSettings.SectionName).Get<CorsSettings>() ?? new CorsSettings();
    var (corsOrigins, usedFallback) = CorsOriginResolver.ResolveForProduction(app.Configuration, corsSettings);

    if (usedFallback)
    {
        app.Logger.LogWarning(
            "Cors__AllowedOrigins is not configured. Using temporary fallback CORS origins: {Origins}. " +
            "Set Cors__AllowedOrigins to your Vercel URL after deploying the frontend.",
